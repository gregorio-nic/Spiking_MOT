...
class BaseTrainer(LightningModule):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx=0):
        ...
        ys_pred = self.network(xs)
        acc = self.network.compute_multi_acc_metrics(self.network.spk_rec, ys)

        #wandb.log({"acc": acc,"loss": res.item()})
    ...
    def fit(self, train_dataset=None, valid_dataset=None, artifact_dir=None, nb_ckpts=1, verbose=0, **kwargs):
        ...
        def get_trainer():
            callbacks = [EarlyStopping(monitor='val_ce', patience=20)] if self.early_stopping else []
        ...
        trainer = get_trainer()
        #print("STARTING BEST LOSS: ", self.best_train_loss)
        trainer.fit(self)
        #print("END BEST LOSS: ", self.best_train_loss)
        self.fitted = True
        #wandb.finish()
        return self
