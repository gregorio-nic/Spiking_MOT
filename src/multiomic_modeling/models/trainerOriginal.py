...
from multiomic_modeling.models.models import MultiomicPredictionModelOriginal
...
class MultiomicTrainerOriginal(BaseTrainer):
    name_map = dict(
        mo_model = MultiomicPredictionModelOriginal
    )
    ...
    def init_network(self, hparams):
        self.network = MultiomicPredictionModelOriginal(**hparams).float()
    ...
    def load_average_weights(self, file_paths) -> None:
        ...
        for file_path in file_paths:
            state_new = MultiomicTrainerOriginal.load_from_checkpoint(file_path, map_location=self.device).state_dict()
            keys = state.keys()
    ...
    def run_experiment(model_params: dict, 
        ...
        model = MultiomicTrainerOriginal(Namespace(**model_params))
