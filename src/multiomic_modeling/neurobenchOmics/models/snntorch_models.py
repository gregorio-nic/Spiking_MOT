...
from multiomic_modeling.models.models import MultiomicPredictionModel, MultiomicPredictionModelOriginal
...
class SNNTorchModel(NeuroBenchModel):
    ...
    def __call__(self, data):
        ...
        spk_out = None
        if isinstance(self.net, MultiomicPredictionModel):
            spk_out = self.net(data)
            spk_out = self.net.spk_rec
        elif isinstance(self.net, MultiomicPredictionModelOriginal):
            spk_out = self.net(data)       
        else:
            raise Exception("Invalid MultiOmics model")
